...
- シーン: 3つの相互接続されたマップ (FIELD, VILLAGE, CAVE)
- 言語: 日本語（すべてのUI/メッセージ）
- コード規模: 約3,157行のJavaScript（10モジュール）
...
- **画像が表示されない**: ブラウザのコンソールでCORSエラーを確認してください。必ずローカルサーバー経由でアクセスしてください。
- **操作が効かない**: ページを更新（F5）してみてください。
- **ゲームが動かない**: ブラウザのコンソール（F12）でエラーメッセージを確認してください。
...
├── style.css               # スタイルシート
├── sketch.js               # p5.js メインループ（40行）
├── game_state.js           # ゲーム状態管理（1,039行）
├── map_data.js             # マップ定義とシーン遷移（258行）
├── entities.js             # 敵/NPCシステムとパスファインディング（460行）
├── renderer.js             # キャンバスレンダリングとUI表示（496行）
├── input.js                # 入力処理とゲームコントロール（315行）
├── combat.js               # ターン制バトルシステム（148行）
├── shop.js                 # ショップ/商人インタラクション（173行）
├── inn.js                  # 宿屋システム（68行）
├── utils.js                # ユーティリティ関数（A*パスファインディング含む）（160行）
├── assets/
│   ├── tiles.png           # タイルセットスプライト（48x48/タイル、8列）
│   ├── actors.png          # プレイヤー/NPCスプライト
│   ├── enemies.png         # 敵スプライト
│   └── objects_interactable.png  # 宝箱などのインタラクティブオブジェクト
├── README.md               # このファイル
...
| ファイル | 行数 | 責務 |
|---------|------|------|
| `game_state.js` | 1,039 | プレイヤー統計、インベントリ、フラグ、占有マップ、アイテム取引の中央管理 |
| `entities.js` | 460 | 敵のスポーン、AIパスファインディング、移動、レンダリング、ワールドとのインタラクション |
| `renderer.js` | 496 | タイルマップ、エンティティ、UIパネル、オーバーレイ、バトル画面、クリア画面の描画 |
| `input.js` | 315 | 移動、戦闘アクション、UIナビゲーション、NPCインタラクションのキーボード処理 |
| `map_data.js` | 258 | 3シーンのタイル定義、スポーン地点、入口/出口設定 |
| `shop.js` | 173 | 商人とのやり取り：購入、売却、価格計算、インベントリチェック |
| `combat.js` | 148 | バトルメカニクス：ダメージ計算、ターンフロー、勝利/敗北処理 |
| `utils.js` | 160 | ユーティリティ関数：A*パスファインディング、距離計算、乱数生成 |
| `inn.js` | 68 | 宿屋主人とのやり取り：宿泊検証とHP回復 |
| `sketch.js` | 40 | p5.jsエントリポイント：アセット読み込み、キャンバス設定、メイン描画ループ |
...
- **モジュール設計**: 10個の独立したIIFEモジュールでグローバル名前空間の汚染を防止
...
- **イベント**: ENTRANCE_VIL, ENTRANCE_CAVE, STAIRS_UP, RUINS
...
このプロジェクトは完全にプレイ可能なUltimaライクRPGで、以下のような潜在的な拡張の堅実な基盤を持っています：
...
プロジェクトライセンスについては、リポジトリ管理者にお問い合わせください。
